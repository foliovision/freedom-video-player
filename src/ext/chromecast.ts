...
import type { PlayerApi, VideoSource } from '../types';
...
freedomplayer(function (api: PlayerApi, root: HTMLElement) {
  if ((false === api.conf.chromecast || '' === api.conf.chromecast) && !api.conf.skin_preview) return;
  ...
  const conf = (api.conf.chromecast || {}) as Record<string, any>;
  ...
  api.createChromecastButton = function (): void {
    ...
    if (!engineEl) common.prepend(common.find('.fp-player', root)[0] || root, chromeCastEngine);
    else engineEl.parentNode!.insertBefore(chromeCastEngine, engineEl);
  };

  if (api.conf.skin_preview && api.conf.chromecast) api.createChromecastButton!();
  ...
  document.addEventListener('fv_player_chromecast_loaded', () => {
    isAvailable = true;
    if (getMedia()) api.createChromecastButton!();
    else common.removeNode(trigger);
  });

  document.addEventListener('fv_player_chromecast_video_loaded', (e: Event) => {
    if (root.getAttribute('data-freedomplayer-instance-id') !== (e as CustomEvent).detail) {
      destroy();
  ...
  domOn(root, api.touch_events(), (ev: Event) => {
    if (!(ev.target as HTMLElement)?.closest('.fp-chromecast')) return;
    ev.preventDefault();
    if (session) {
      ...
      destroy();
      api.trigger('pause', [api]);
      if (api.video.time) setTimeout(() => api.seek(api.video.time || 0));
      return;
    }
    if (api.playing) api.pause();
    window.cast.framework.CastContext.getInstance().requestSession().then(
      () => { session = window.cast.framework.CastContext.getInstance().getCurrentSession(); loadMedia(); },
      (err: string) => { if ('cancel' !== err) console.error('Chromecast session ' + err); }
    );
  });

  function getMedia(): VideoSource | false {
    let media: VideoSource | false = false;
    const sources = (api.video as any).sources_fvqs || api.video.sources;
    ...
    if (!media) {
      for (const source of sources || []) {
        if (source.type === 'application/x-mpegurl') { media = source; break; }
      }
    }
    if ((api.video as any).fvhkey && !api.conf.hls_cast) return false;
    if (media) {
      let topQuality: VideoSource | false = false;
      const mp4Qualities = ['fullhd', 'hd', 'md', 'sd'];
      for (const quality of mp4Qualities) {
        const re = new RegExp('-' + quality);
        for (const source of (api.video as any).sources_fvqs || []) {
          if (source.src.match(re) && source.type === 'video/mp4') { topQuality = source; break; }
        }
        if (topQuality) { media = topQuality; break; }
      }
    }
    return media;
  }
  ...
  function loadMedia(): void {
    ...
    common.html(common.find('.fp-chromecast-engine-status', root)[0] as HTMLElement, 'Playing on device ' + receiverName);

    const mediaInfo = new window.chrome.cast.media.MediaInfo(media.src, media.type);
    const request = new window.chrome.cast.media.LoadRequest(mediaInfo);
    if (!api.live) request.currentTime = api.video.time;
    const castSession = window.cast.framework.CastContext.getInstance().getCurrentSession();
    castSession.loadMedia(request).then(onMediaDiscovered, (errorCode: string) => {
      console.log('Chromecast onMediaError: ' + errorCode);
    });
  }

  function onMediaDiscovered(): void {
  ...
  function onChromecastEvent(e: any): void {
    if (session && !api.hijacked) {
      ...
      api.hijack?.({
        ...
        resume() {
          if (api.finished) { api.release?.(); loadMedia(); return; }
          if (remotePlayer.isPaused) remoteController.playOrPause();
        },
        seek(time: number) {
    ...
    if (e.field === 'currentTime') {
      ...
      if (api.paused && e.value === window.chrome.cast.media.PlayerState.PLAYING) api.trigger('resume', [api]);
      else if (api.playing && e.value === window.chrome.cast.media.PlayerState.PAUSED) api.trigger('pause', [api]);
      else if (e.value === window.chrome.cast.media.PlayerState.IDLE) {
        api.trigger('pause', [api]);
        api.trigger('finish', [api]);
      }
      if (api.seeking) {
        if (e.value === window.chrome.cast.media.PlayerState.BUFFERING) waitingForSeek = true;
