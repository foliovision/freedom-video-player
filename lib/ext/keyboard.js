...
flowplayer(function(api, root) {
  ...
  bean.on(root, "wheel", function(e) {
    if( typeof(focused) == "undefined" ) return;

    if( e.target.classList.contains('fp-volume') ) {
      ...
      if (delta > 0) el.volume(api.volumeLevel - 0.15); // going down
      else el.volume(el.volumeLevel + 0.15); // going up
