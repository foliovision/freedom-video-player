...
function slider(root, opts) {
  ...
  bean.on(root, 'mousedown.sld touchstart.sld', function(ev) {
    ...
    bean.on( flowplayer.support.touch ? root : document, 'mousemove.sld touchmove.sld', function(ev) {
      ev.preventDefault();
      api.slide(getMouseValue(ev), true);
    });

    bean.one(document, 'mouseup.sld touchup.sld', function() {
      bean.off( flowplayer.support.touch ? root : document, 'mousemove.sld touchmove.sld');
    });
  });
