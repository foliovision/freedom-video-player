...
freedomplayer(function(api, root) {
  ...
  var waiting = find('waiting'),
   ...
   //Interaction events
   var mouseoutTimeout = freedomplayer.support.touch ? conf.mouseoutTimeout * 1.7 : conf.mouseoutTimeout;
   ...
   bean.on(root, "mouseenter mouseleave", function(e) {
      ...
      hover(is_over && api.was_played);

      if (is_over) {

         var reg = function() {
            // Show hover controls if video was already played
            hover(api.was_played);
         ...
         hovertimer = setInterval(function() {
            if (new Date() - api.ui_lastMove > mouseoutTimeout ) {
