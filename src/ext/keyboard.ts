...
let focused: PlayerApi | undefined;
let focusedRoot: HTMLElement | undefined;

freedomplayer(function (api: PlayerApi, root: HTMLElement) {
  if (!api.conf.keyboard) return;

  domOn(document, 'keydown.fp', (e: Event) => {
    ...
    if (focusedRoot && (common.hasClass(focusedRoot, 'no-controlbar') || common.hasClass(focusedRoot, 'is-cva'))) return;
    ...
    if ([63, 187, 191].indexOf(key) !== -1) {
      if (focusedRoot) common.toggleClass(focusedRoot, 'is-help');
      return;
    }
    ...
    if (!metaKeyPressed && el.ready) {
      ...
      if (ev.shiftKey) {
        ...
        else if (key === 78) el.next?.();
        else if (key === 80) el.prev?.();
        return;
      }

      // Number keys 1-9 → seek to percentage
      if (key < 58 && key > 47) { el.seekTo(key - 48); return; }

      switch (key) {
        case 38: case 75: el.volume(el.volumeLevel + 0.15); break;
        case 40: case 74: el.volume(el.volumeLevel - 0.15); break;
        case 39: case 76: el.seeking = true; el.manual_seeking = true; el.seek((api.video.time || 0) + 5); break;
        case 37: case 72: el.seeking = true; el.manual_seeking = true; el.seek((api.video.time || 0) - 5); break;
        case 32:
          if (api.playing) el.manual_pause = true;
          else if (api.paused) el.manual_resume = true;
          el.toggle();
          break;
        ...
        case 67: {
          ...
          const currentEl = focusedRoot?.querySelector('.fp-dropdown li.active[data-subtitle-index]') as HTMLElement | null;
          let currentSubtitles = currentEl ? Number(currentEl.dataset.subtitleIndex) : -1;
          currentSubtitles++;
          if (currentSubtitles > api.video.subtitles.length - 1) currentSubtitles = -1;
          api.trigger('subtitles-switched', [currentSubtitles]);
          break;
        }
        case 190: case 188:
          if (api.playing) { el.manual_pause = true; el.pause(); }
  ...
  domOn(root, 'mouseenter.fp mouseleave.fp focus.fp', (e: Event) => {
