...
if (support.touch || isIeMobile) {
   ...
   freedomplayer(function(player, root) {
      ...
      bean.on( common.find('svg', forward)[0], 'click', function() {
      ...
      bean.on( common.find('svg', rewind)[0], 'click', function() {
         if (player.disabled) return;
      ...
      bean.on(root, 'touchend click', function(e) {
         ...
         if ( player.was_played && !mouseover || common.hasClass(e.target, 'fp-ui') || common.hasClass(e.target, 'fp-engine')) {
            common.toggleClass(root, 'is-mouseover', !mouseover);
            common.toggleClass(root, 'is-mouseout', mouseover);
            e.preventDefault();
            e.stopPropagation();

            if( mouseover ) {
               player.ui_lastMove = new Date();
            }
