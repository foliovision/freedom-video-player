   ...
   freedomplayer(function(player, root) {
      ...
      bean.on(root, 'touchend click', function(e) {
         ...
         if ( lastTap && double_tap_duration > 20 && double_tap_duration < doubleTapTime && ( rewind || forward ) ) {
            ...
            // Not if already seeking
            if ( common.hasClass(root, 'is-mobile-seeking') ) {
               return;
            }
         ...
         if ( player.was_played ) {
            if ( !mouseover || common.hasClass(e.target, 'fp-ui') || common.hasClass(e.target, 'fp-engine') || common.hasClass(e.target, 'fp-header') || used_mobile_seeking ) {
               ...
               hoverInt = setTimeout( function() {
                  ...
                  // Hide mobile seeking controls
                  } else if ( common.hasClass(root, 'is-mobile-seeking') ) {
                     common.removeClass(root, 'is-mobile-seeking');

                     // User paused the video, show the controls
                     if ( common.hasParent( e.target, '.fp-pause') ) {
                        player.hover( true );
                     }

                  // Show/hide the UI
                  } else {
                     // touchend uses e.changedTouches, click uses e.clientX/Y
                     var x = e.changedTouches && e.changedTouches[0] && e.changedTouches[0].clientX  ? e.changedTouches[0].clientX : e.clientX,
                        y = e.changedTouches && e.changedTouches[0] && e.changedTouches[0].clientY  ? e.changedTouches[0].clientY : e.clientY;

                     // Show controls if...
                     if (
                        // ...player is not in fullscreen
                        ! player.isFullscreen ||
                        // ...or if the tap is not in the left/right 1/6 of the screen and not in the top/bottom 20px
                        x > ( window.innerWidth / 6 ) && x < ( 5 * window.innerWidth / 6 ) && y > 20 && y < window.innerHeight - 20 ||
                        // ...tapping the area where timeline is
                        common.hasClass( e.target, 'fp-timeline' ) || common.hasClass( e.target.parentNode, 'fp-timeline' )
                     ) {
