...
const freedomplayer = function (fn, opts, callback) {
    if (typeof fn === 'function') {
        extensions.push(fn);
        return;
    }
    if (typeof fn === 'number' || typeof fn === 'undefined') {
        return instances[fn || 0];
    }
    if (fn.nodeType) {
        const el = fn;
    ...
    if (fn.jquery) {
        return freedomplayer(fn[0], opts, callback);
    }
    if (typeof fn === 'string') {
        const el = common.find(fn)[0];
...
Object.assign(freedomplayer, {
    ...
    set(key, value) {
        if (typeof key === 'string')
            freedomplayer.conf[key] = value;
        else
            Object.assign(freedomplayer.conf, key);
    },
    registerExtension(js, css) {
...
if (typeof window.jQuery !== 'undefined') {
    ...
    const videoTagConfig = function (videoTag) {
        if (!videoTag.length)
            return {};
        const clip = videoTag.data() || {};
        const conf = {};
        $.each(['autoplay', 'loop', 'preload', 'poster'], function (_i, key) {
            const val = videoTag.attr(key);
            if (val !== undefined && ['autoplay', 'poster'].indexOf(key) !== -1)
                conf[key] = val ? val : true;
            else if (val !== undefined)
                clip[key] = val ? val : true;
        });
...
function initializePlayer(element, opts, callback) {
    if (opts && opts.embed)
        opts.embed = Object.assign({}, freedomplayer.defaults.embed, opts.embed);
    ...
    const root = element;
    const conf = Object.assign({}, freedomplayer.defaults, freedomplayer.conf, opts);
    let storage = {};
    const originalClass = root.className;
    let engine;
    let seeking_indicator_interval;
    ...
    const storedMuted = storage.muted;
    const storedVolume = storage.volume;
    ...
    const api = ({
        ...
        load(video, cb) {
            ...
            const resolved = Object.assign({}, urlResolver.resolve(clip, conf.clip?.sources));
            if (api.playing || api.engine)
                resolved.autoplay = true;
            const engineImpl = selectEngine(resolved);
            if (!engineImpl) {
                setTimeout(() => { api.trigger('error', [api, { code: 5 }]); });
                return api;
            }
            if (!engineImpl.engineName)
            ...
            const picked = engine.pick((resolved.sources || []).filter((source) => {
                if (!source.engine)
                    return true;
                return source.engine === engine.engineName;
            }));
            if (picked)
                Object.assign(resolved, picked);
            if (resolved.src) {
                const e = api.trigger('load', [api, resolved, engine], true);
        ...
        volume(level, skipStore) {
            if (freedomplayer.support.android && level > 0)
                level = 1;
            if (level === 0) {
        ...
        registerExtension(jsUrls, cssUrls) {
    ...
    const selectEngine = (clip) => {
        ...
        clip.sources?.some(source => {
            const e = engines.filter(engine => {
                if (source.engine && source.engine !== engine.engineName)
                    return false;
    ...
    if (!root.getAttribute('data-freedomplayer-instance-id')) {
        ...
        api.on('boot', () => {
            ...
            if (conf.splash)
                common.find('video', root).forEach(common.removeNode);
        ...
        }).on('load', (_e, _api, video) => {
            if (conf.splash) {
                common.find('.freedomplayer.is-ready,.freedomplayer.is-loading').forEach(el => {
                    const playerId = el.getAttribute('data-freedomplayer-instance-id');
                    if (playerId === root.getAttribute('data-freedomplayer-instance-id'))
                        return;
            ...
            api.ready = is_ready;
            api.splash = !is_ready;
        }).on('progress', (_e, _api, time) => {
            if (!api.was_played) {
                api.was_played = true;
                common.addClass(root, 'was-played');
            }
            api.video.time = time;
        }).on('buffer', (_e, _api, buffered) => {
        ...
        }).on('volume', (_e, _api, level) => {
            api.volumeLevel = Math.round(level * 100) / 100;
            if (api.muted && level)
                api.mute(false);
        }).on('beforeseek seek', (e) => {
            ...
            if (e.type === 'seek')
                setTimeout(() => { api.manual_seeking = false; }, 0);
        }).on('ready pause resume unload finish stop', (e) => {
