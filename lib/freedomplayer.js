...
/* freedomplayer()  */
var freedomplayer = module.exports = function(fn, opts, callback) {
  ...
  if (fn.nodeType) { // Is an element
    if (fn.getAttribute('data-freedomplayer-instance-id') !== null) { // Already freedomplayer instance
      return instances[fn.getAttribute('data-freedomplayer-instance-id')];
    }
    if (!opts) return; // Can't initialize without data
    return initializePlayer(fn, opts, callback);
  }
  if (fn.jquery) return freedomplayer(fn[0], opts, callback);
  if (typeof fn === 'string') {
    var el = common.find(fn)[0];
    return el && freedomplayer(el, opts, callback);
  }
};

extend(freedomplayer, {
   ...
   engine: function(name) {
     return freedomplayer.engines.filter(function(e) { return e.engineName === name; })[0];
   },
   ...
   set: function(key, value) {
      if (typeof key === 'string') freedomplayer.conf[key] = value;
      else extend(freedomplayer.conf, key);
   },
...
if (typeof window.jQuery !== 'undefined') {
  ...
  // Avoid native unload listeners (Permissions-Policy: unload=())
  $.event.special.unload = {
    setup: function() { return true; },
    teardown: function() { return true; }
  };

  // auto-install (any video tag with parent .freedomplayer)
  $(function() {
  ...
  $.fn.freedomplayer = function(opts, callback) {
...
function initializePlayer(element, opts, callback) {
  if (opts && opts.embed) opts.embed = extend({}, freedomplayer.defaults.embed, opts.embed);

  var supportLocalStorage = false;
  try {
    if (typeof freedomplayer.conf.storage === 'undefined' && typeof window.localStorage == "object") {
      window.localStorage.freedomplayerTestStorage = "test";
  ...
  var root = element,
      conf = extend({}, freedomplayer.defaults, freedomplayer.conf, opts),
      storage = {},
      originalClass = root.className,
      lastSeekPosition,
      engine,
      seeking_indicator_interval,
      urlResolver = new URLResolver();

      common.addClass(root, 'is-loading');
      common.toggleClass(root, 'no-flex', !freedomplayer.support.flex);
      try {
         storage = freedomplayer.conf.storage || (supportLocalStorage ? window.localStorage : storage);
      } catch(e) {}

      conf.volume = storage.muted === "true" ? 0 : conf.volume !== freedomplayer.defaults.volume ? conf.volume : !isNaN(storage.volume) ? storage.volume : conf.volume;

      conf.debug = !!storage.freedomplayerDebug || conf.debug;
      ...
      var isRTL = freedomplayer.is_rtl;

      // This is slow, do it only once
      if ( ! freedomplayer.did_rtl_check ) {
         freedomplayer.did_rtl_check = true;
 
         isRTL = (root.currentStyle && root.currentStyle.direction === 'rtl') || (window.getComputedStyle && window.getComputedStyle(root, null) !== null && window.getComputedStyle(root, null).getPropertyValue('direction') === 'rtl');
      ...
      var api = {
         ...
         load: function(video, callback) {
            ...
            if (!engineImpl) return setTimeout(function() { api.trigger("error", [api, { code: 5 }]); }) && api;
         ...
         */
         seek: function(time, callback) {
            ...
            if ( time < api.get_video_start() ) {
               time = api.get_video_start();
            }

            var absolute_duration = api.get_video_duration() + api.get_video_start();
            if ( time > absolute_duration ) {
               time = absolute_duration;
            }
            ...
            if (api.ready) {
               ...
               if (!ev.defaultPrevented) {
                  ...
                  api.seeking = false;
                  api.manual_seeking = false;

                  // Add the class with a delay to avoid flickering
                  if ( api.seeking ) {
                     clearInterval( seeking_indicator_interval );

                     seeking_indicator_interval = setTimeout( function() {
         ...
         */
         ...
         mute: function(flag, skipStore) {
            ...
            if (!skipStore) {
               ...
               if ( !freedomplayer.support.volume && !flag ) {
                  storage.volume = 1;
               }
         ...
         volume: function(level, skipStore) {
            ...
            // Disabling volume control for Android
            if ( freedomplayer.support.android && parseFloat(level) > 0 ) {
               level = 1;
            }

            // Fix if somebody has localStorage.volume = 0;
            if (level == 0) {
               api.mute(true, skipStore);
               return;
            }
         ...
         shutdown: function() {
           ...
           bean.off(root);
           delete instances[root.getAttribute('data-freedomplayer-instance-id')];
         ...
         is_playlist: function() {
            return api.conf.playlist && api.conf.playlist.length > 1;
         },

         is_last_video: function() {
            return api.is_playlist() && api.get_video_index() == api.conf.playlist.length - 1;
         },

         get_video_index: function() {
            if( !api.video.index ) {
              return 0;
            }

            return api.video.index;
         },

         get_video_duration: function() {
            if ( api.get_custom_duration ) {
               return api.get_custom_duration();
            } else {
               return api.video.duration;
            }
         },

         get_video_start: function() {
            if ( api.get_custom_start ) {
               return api.get_custom_start();
            } else {
               return 0;
            }
         },

         get_video_end: function() {
            if ( api.get_custom_end ) {
               return api.get_custom_end();
            } else {
               return api.video.duration;
            }
         },
      ...
      freedomplayer.extensions.forEach(function(i) {
      ...
      var selectEngine = function(clip) {
        var engine;
        var engines = freedomplayer.engines;
        if (conf.engine) {
        ...
        if (conf.enginePreference) engines = freedomplayer.engines.filter(function(one) { return conf.enginePreference.indexOf(one.engineName) > -1; }).sort(function(a, b) {
      ...
      /*** Behaviour ***/
      if (!root.getAttribute('data-freedomplayer-instance-id')) { // Only bind once
         root.setAttribute('data-freedomplayer-instance-id', playerCount++);

         root.setAttribute('tabindex', 0);

         api.on('boot', function() {
            var  support = freedomplayer.support;
            ...
            // unload others
            if (conf.splash) {
              common.find('.freedomplayer.is-ready,.freedomplayer.is-loading').forEach(function(el) {
                var playerId = el.getAttribute('data-freedomplayer-instance-id');
            ...
            // see https://github.com/freedomplayer/freedomplayer/issues/479
           ...
           api.splash = !is_ready;
            ...
            // We track if the video was-played to be able to suppress the hover controls on mobile
            ...
            if(e.type == 'seek') setTimeout(function() { api.manual_seeking = false; }, 0);
            ...
            if ( e.type == 'pause' ) {
               setTimeout( function() {
                  api.manual_pause = false;
               }, 0 );

            } else if( e.type == 'resume' ) {
               setTimeout( function() {
                  api.manual_resume = false;
               }, 0 );
            }
