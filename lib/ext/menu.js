...
freedomplayer(function(api, root) {
  api.showMenu = function(menu, triggerElement) {
    ...
    setTimeout(function() {
      ...
      /**
       * We need touchstart below as otherwise the mobile controls overlay
       * would block the menu closing
       * 
       * We need touchend for Modulobox lightbox, see api.touch_events()
       */
      bean.one(document, 'click touchstart touchend', function() {
        api.hideMenu(menu);
      });
    });
