...
var freedomplayer = require('../freedomplayer')
  ...
  , bean = freedomplayer.bean
  , extend = freedomplayer.extend;

var desktopSafari = support.browser.safari && !support.iOS;
// HTML5 --> Freedom Player event
var EVENTS = {
  ended: 'finish',
  pause: 'pause',
  play: 'resume',
  ...
  webkitendfullscreen: !freedomplayer.support.inlineVideo && 'unload',
  progress: 'buffer'
};
...
function html5factory(engineName, player, root, canPlay, ext) {
  var api = common.findDirect('video', root)[0] || common.find('.fp-player > video', root)[0]
    ...
    , timer
    , volumeLevel
    , click_to_unmute_dismiss
    , self;

  function try_muted_autoplay(p) {
    if ( p && p.catch ) {
      var recoverAutoplay = function(err) {
        ...
        if ( ! common.hasClass( root, 'is-audio' ) ) {
          if ( click_to_unmute_dismiss ) {
            click_to_unmute_dismiss();
          }

          click_to_unmute_dismiss = player.message( '<span class="fp-icon fp-volumebtn-notice"></span> ' + conf.msg. click_to_unmute, 10000, { className: 'fp-message-muted' } );
        }

        bean.on( root, 'click', '.fp-message-muted', function(e) {
          player.mute(false);
          player.volume(1);
  ...
  return self = {
    ...
    load: function(video) {
      ...
      // Obtaining video URL via Ajax
      if(
        window.FV_Player_Pro && window.fv_player_video_parse &&
        !root.getAttribute( 'fvhkey' ) && FV_Player_Pro.is_dynamic(video.src)
      ) {
        fv_player_video_parse( video, this, player, root );
        
        if( window.fv_player_pro_dammit[ root.id ] ) return false; //  exp: if the video was getting loaded previously, we need to bail!

        if( support.inlineVideo ) window.fv_player_pro_dammit[ root.id ] = true;
      }
      ...
      // This lets plugins force crossOrigin for the video element which is needed if AudioWorklet is going to be used.
      if ( conf.video_cross_origin ) {
        common.attr(api, 'crossorigin', 'anonymous');
      } else {
        common.attr(api, 'crossorigin', false);
      }
      ...
      if (support.subtitles && conf.nativesubtitles && video.subtitles && video.subtitles.length) {
        ...
        if ( ! support.iOS || parseInt( support.iOS.version ) < 18 ) {
      ...
      if (video.loop || conf.loop) {
        player.on('finish.loophack', function() {
          ...
          // Hide the play button animation
          // TODO: This could be done in a better way, perhaps using a flag for user interraction
          setTimeout(function() {
            var play_button = common.find(".fp-play", root);
            if ( play_button && play_button[0] ) {
              common.removeClass( play_button[0], 'fp-visible');
            }
          }, 0 );
        });
      }
      ...
      var event_name = 'scroll.preloadviewport-' + root.getAttribute('data-freedomplayer-instance-id');

      var preloadCheck = function() {
        // Use api.force_preload to bypass viewport check if preloading videos below the fold.
        ...
        bean.off(document, event_name );
      };
      bean.off(document, event_name );
      bean.on(document, event_name, function() {
    ...
    unload: function() {
      ...
      var instanceId = root.getAttribute('data-freedomplayer-instance-id');

      if ( api && api.listeners && api.listeners[instanceId] ) {
      ...
      var l = function(e) {
        ...
        case 'ready':
          ...
          if ((!api.duration || api.duration === Infinity) && !player.live) {
            ...
            // If we see 2 progress events on Safari or iPhone for HLS, we can assume it's a live stream
            if ( ( support.browser.safari || support.iOS ) && api.type && api.type.match(/application\/x-mpegurl/i) ) {

              api.addEventListener('progress', function() {
                api.addEventListener('progress', function() {
                  if ( api.ios_live_stream_ready_forced || player.ready ) return;
            ...
            // TODO: When you check here https://github.com/flowplayer/flowplayer/issues/1318 you will see that it's only trying to not show invalid duration in the UI. Perhaps we should let the event through and let the UI handle display of NaN duration in a better way.
            return player.debug('No duration and VOD setup, not sending ready event');
          }
        ...
        case 'progress': case 'seek':
          ...
          // Remove Click to Unmute message if video has no audio
          if (
            click_to_unmute_dismiss && (
              ! api.mozHasAudio &&
              ! Boolean( api.webkitAudioDecodedByteCount ) &&
              ! Boolean( api.audioTracks && api.audioTracks.length )
            )
          ) {
            click_to_unmute_dismiss();
          }
        ...
        case 'volume':
          ...
          if ( arg > 0 && click_to_unmute_dismiss ) {
            click_to_unmute_dismiss();
          }
        ...
        case 'error':
          //  if we change the video src using Ajax, the original video src will still throw error on mobile devices
          if( window.FV_Player_Pro && FV_Player_Pro.is_dynamic(api.src) ) {
          ...
          //  when loading Vimeo DASH we must let the error through as that's the right moment to use the other engine, but let's not make a big deal out of it
