...
freedomplayer(function(api, root) {
  ...
  var waiting = find('waiting'),
     ...
     if (noToggle) return;
      ...
      if (is_over) {
         ...
         hovertimer = setInterval(function() {
            if (new Date() - api.ui_lastMove > conf.mouseoutTimeout) {
   ...
   bean.on(root, "click.player", function(e) {
      ...
      // On mobile you need to tap the play/pause button, on desktop you can just click anywhere to pause
      if (
         common.hasParent(e.target, '.fp-play,.fp-pause') ||
         !support.touch && (common.hasClass(e.target, 'fp-ui') || common.hasClass(e.target, 'fp-engine'))
      ) {
