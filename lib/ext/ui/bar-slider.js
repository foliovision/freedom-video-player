...
function slider(root, opts) {
  ...
  bean.on(root, 'mousedown.sld touchstart.sld', function(ev) {
    ...
    bean.on( freedomplayer.support.touch ? root : document, 'mousemove.sld touchmove.sld', function(ev) {
      ev.preventDefault();
      api.slide(getMouseValue(ev), true);
    });

    bean.one(document, 'mouseup.sld touchup.sld', function() {
      bean.off( freedomplayer.support.touch ? root : document, 'mousemove.sld touchmove.sld');
    });
  });
