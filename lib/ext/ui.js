...
freedomplayer(function(api, root) {
   ...
   // Only create fp-ratio element if it doesn't exist
   if ( ! common.find('.fp-ratio', root)[0] ) {
      var ratio = common.createElement( 'div', { className: 'fp-ratio', style: 'display: none' } );
      root.appendChild( ratio );
   }

   /**
    * Create a placeholder video if there is no video in the player.
    * We do this for iOS, so that we can unmute the video tag when user
    * clicks to unmute any other video on the page. Then other videos
    * will autoplay with sound reliably.
    */
   var placeholder_video = false;
   if ( support.iOS &&! common.findDirect('video', root)[0] && ! common.find('.fp-player > video', root)[0] ) {
      placeholder_video = document.createElement('video');
      placeholder_video.muted = true;
      placeholder_video.className = 'fp-placeholder-video';
      placeholder_video.style = 'display: none';
      root.appendChild( placeholder_video );
   }
   ...
   // TODO: Only insert SVG which is actually going to be used based on the root classes (fp-edgy and so on)
   var ui = common.createElement('div', {className: 'fp-ui', style: 'display: none'}, '\
         ...
         <div class="fp-controls">\
            ...
            <a class="fp-fullscreen fp-icon">\
               <svg class="fp-fullscreen-enter" xmlns="http://www.w3.org/2000/svg" viewBox="-2 -2 40 30" fill="none" xmlns:xlink="http://www.w3.org/1999/xlink"><path d="M1 9V1h12" fill="none" /><path d="M1 9V1h12" transform="matrix(1 0 0 -1 0 26)" fill="none" /><path d="M1 9V1h12" transform="matrix(-1 0 0 1 36 0)" fill="none" /><path d="M1 9V1h12" transform="rotate(180 18 13)" fill="none" /></svg>\
               <svg class="fp-fullscreen-exit" xmlns="http://www.w3.org/2000/svg" viewBox="-1 -2 40 30" xmlns:xlink="http://www.w3.org/1999/xlink"><path d="M1 9V1h12" transform="translate(22 17)" fill="none" /><path d="M1 9V1h12" transform="matrix(-1 0 0 1 13 17)" fill="none" /><path d="M1 9V1h12" transform="rotate(180 6.5 4.5)" fill="none" /><path d="M1 9V1h12" transform="matrix(1 0 0 -1 22 9)" fill="none" /></svg>\
            </a>\
  ...
  var waiting = find('waiting'),
      ...
      origRatio, // obtail later in delayed_cals
      play = find('play'),
      pause = find('pause'),
   ...
   // This is used to batch DOM updates
   if (typeof window.requestAnimationFrame === 'function') {
      window.requestAnimationFrame( delayed_calls );
   } else {
      delayed_calls();
   }
   ...
   function hover( is_over ) {
      ...
      if (is_over) {
         ...
         var reg = function(e) {
            ...
            if ( ! support.touch && 'number' === typeof( e.clientY ) ) {
               var rect = ui.getBoundingClientRect()
               var y = e.clientY - rect.top - document.documentElement.scrollTop;

               // Show header if mouse is over the top 1/3 of the player, used in fullscreen
               common.toggleClass(root, 'is-mouseover-top', y < ( 0.33 * ui.clientHeight ) );

               // Show fullscreen exit hint if mouse is over the top 5% of the player and on the right 5% of the player
               if ( common.hasClass(root, 'is-fullscreen') && y < ( 0.05 * ui.clientHeight ) && e.clientX > ( ui.clientWidth * 0.95 ) ) {
                  ...
                  // Show control bar if mouse is over the bottom 1/5 of the player, used in fullscreen
   ...
   try {
      if (!conf.fullscreen) fullscreen.style.display = 'none';
   ...
   api.on("ready", function(ev, api, video) {
      // Make sure the slider width is calculated so that it can detect cursor position correctly
      timelineApi.calc();

      var duration = api.get_video_duration();
   ...
   // buffer
      ...
      var video = api.video,
      max = video.buffer / api.get_video_duration();

      if (!video.seekable && support.seekable) timelineApi.max(api.conf.live ? Infinity : max);

      // The video might be becoming seekable later
      if ( video.seekable && support.seekable && timelineApi.getMax() ) timelineApi.max(1);
      ...
      buffered.forEach(function(b, i) {
         // subtract if custom start is set
         ...
         if ( api.rtl ) {
            common.css(buffer, {
               right: (100 * b.start / duration) + '%',
               width: (100 * (b.end - b.start) / duration) + '%'
            });
         } else {
            common.css(buffer, {
               left: (100 * b.start / duration) + '%',
               width: (100 * (b.end - b.start) / duration) + '%'
            });
         }
   ...
   // progress
      ...
      if ( api.get_custom_time ) {
         time = api.get_custom_time( time);
      }
      ...
      // Only show the pause button if the video status will not change to finished or playing (in case of a loop) in the next event callback
      // TODO: This could be done in a better way, perhaps using a flag for user interraction
      setTimeout(function() {
         if ( api.finished || api.playing ) {
            return;
         }
   ...
   // misc
      ...
      if (error) {
         ...
         var message = error.custom_message || (api.engine && api.engine.engineName || 'html5') + ": " + conf.errors[code];

         var dismiss = api.message( message );
   ...
   bean.on(root, touch_events, function(e) {
      ...
      // However click/tap anywhere in the player should only play the video on mobile if it was not yet played or if the controls are set to not show
      if (
         common.hasParent(e.target, '.fp-play,.fp-pause') ||
         ( !api.was_played || !support.touch || common.hasClass( root, 'no-controlbar' ) ) && (common.hasClass(e.target, 'fp-ui') || common.hasClass(e.target, 'fp-engine')) ||
         typeof api.video.click != "undefined" ||

         // Splash end image
         common.hasClass(e.target, 'wpfp_custom_background')
      ) {
         if (e.preventDefault) e.preventDefault();
         if ( api.playing ) {
            api.manual_pause = true;
         } else if ( api.paused ) {
            api.manual_resume = true;
         }
         return api.toggle();
      }
   });
   ...
   bean.on(root, 'mousemove touchmove', '.fp-timeline', function(ev) {
      ...
      var delta = x - common.offset(timeline).left,
         ...
         video = api.video,
         duration = api.get_video_duration() - (video.seekOffset === undefined ? 0 : video.seekOffset),
         seconds = (api.rtl ? 1 - percentage : percentage) * duration;
      if (percentage < 0) return;

      var currentChapter = "",
         index = api.get_video_index();

      if ( api.fv_timeline_chapters_data && api.fv_timeline_chapters_data[ index ] ) {
         api.fv_timeline_chapters_data[ index ].forEach( function(chapter) {
            var startTime = chapter.startTime - api.get_video_start(),
            endTime = chapter.endTime - api.get_video_start();

            if (seconds >= startTime && seconds <= endTime) {
               currentChapter = chapter.line;
            }
         });
      }

      var tooltipText = format(seconds);
      if ( currentChapter ) {
         tooltipText = tooltipText + ' ' + currentChapter;
      }
      ...
      if ( timelineTooltip.offsetWidth ) {
         left -= timelineTooltip.offsetWidth / 2;
      }

      if (left < 0) left = 0;
      if (left > common.width(timeline) - timelineTooltip.offsetWidth ? timelineTooltip.offsetWidth : 0 ) left = false;
   ...
   /* controlbar elements */
   bean.on(root, touch_events, '.fp-volumebtn', function() {
      api.mute();

      if ( ! api.muted ) {
         // Unmute all placeholder videos
         document.querySelectorAll( 'video.fp-placeholder-video' ).forEach( video => {
            video.muted = false;
         });
      }
   });
   ...
   bean.on(timeline, 'slide', function( val ) {
      var duration = api.get_video_duration(),
        seek_to_seconds = duration ? api.get_video_start() + val * duration : 0;

      api.seeking = true;
      api.manual_seeking = true;

      if ( api.custom_seek ) {
         api.custom_seek( seek_to_seconds );
      } else {
         api.seek( seek_to_seconds );
      }
   });
   ...
   function check_size() {
      ...
      if (typeof window.requestAnimationFrame === 'function') {
         requestAnimationFrame( player_size_classes );
      } else {
         player_size_classes();
      }
   }

   function player_size_classes() {
      var playerEl = common.find('.fp-player', root)[0] || root,
         width = playerEl.clientWidth;

      if ( conf.ratio > 1 ) {
         common.removeClass(root, 'is-tiny');
         common.removeClass(root, 'is-small');
      } else {
