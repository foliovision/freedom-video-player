...
if (support.touch) {
    freedomplayer(function (player, root) {
        ...
        const isAndroid = !!(android && !android.firefox);
        ...
        const fullscreen = common.find('.fp-fullscreen', root)[0];
        let fullscreenExitHintCount = 0;
        let fullscreenExitHintTimer = null;
        let seekTo = 0;
        let intHideForwardRewind;
        if (!common.hasClass(root, 'no-controlbar')) {
            ...
            domOn(forward, player.touch_events(), (e) => {
                if (player.disabled)
                    return;
                e.preventDefault();
                e.stopPropagation();
            ...
            domOn(rewind, player.touch_events(), (e) => {
                if (player.disabled)
                    return;
                e.preventDefault();
                e.stopPropagation();
        ...
        function hideOverlay() {
            ...
            intHideForwardRewind = setTimeout(() => {
                if (timeline && common.hasClass(timeline, 'is-fp-dragging')) {
                    hideOverlay();
                    return;
                }
        ...
        if (player.sliders?.timeline)
            player.sliders.timeline.disableAnimation();
        let hasMoved = false;
            ...
            if (lastTap && doubleTapDuration > 20 && doubleTapDuration < doubleTapTime && (isRewind || isForward)) {
                if (player.video?.duration) {
                    ...
                    if (isRewind) {
                        ...
                        seekTarget += 10;
                        if (seekTarget > (player.video.duration || 0))
                            seekTarget = player.video.duration || 0;
                    }
                    common.addClass(root, 'is-mobile-seeking');
        ...
        domOn(root, 'touchend click', (e) => {
            ...
            if (initialClick && player.conf.clickToUnMute && video?.muted && player.conf.autoplay)
                video.muted = false;
            initialClick = false;
            const usedMobileSeeking = !!(target.closest('.fp-rewind') || target.closest('.fp-forward'));
            const doNotShowOverlay = common.hasClass(root, 'is-mouseover') ||
                ...
                if (!doNotShowOverlay || target.matches('.fp-ui,.fp-engine,.fp-header') || usedMobileSeeking) {
                    ...
                    hoverInt = setTimeout(() => {
                        if (usedMobileSeeking) {
                            ...
                            const y = (e.changedTouches?.[0]?.clientY ?? e.clientY) - window.scrollY;
                            ...
                            if (!common.hasClass(root, 'is-mouseover') && player.isFullscreen &&
                    ...
                    if (player.playing && common.hasParent(target, '.fp-pause'))
                        return;
                    if (!doNotShowOverlay) {
                        e.preventDefault();
                        e.stopPropagation();
            ...
            if (!player.was_played && !player.splash && common.hasClass(root, 'is-mouseout') && !common.hasClass(root, 'is-mouseover')) {
                setTimeout(() => {
                    if (!player.disabled && !player.playing && !player.splash) {
                        const engine = common.find('video.fp-engine', root)[0];
                        engine?.play();
                    }
                }, 400);
            }
        ...
        if (!support.fullscreen && player.conf.native_fullscreen) {
            ...
            if (typeof testVideo.webkitEnterFullScreen === 'function') {
                const oldFullscreen = player.fullscreen;
                player.fullscreen = function () {
                    const video = common.find('video.fp-engine', root)[0];
                    if (!video)
                        return oldFullscreen?.apply(player);
            ...
            player.bind('ready', () => {
                ...
                player.bind('progress.dur', () => {
                    ...
                    if (duration !== 1) {
                        ...
                        const durationEl = common.find('.fp-duration', root)[0];
                        if (durationEl)
                            durationEl.innerHTML = format(player.get_video_duration());
