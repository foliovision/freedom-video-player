...
engine = function(player, root) {
  ...
  function hlsjsExt(video, api, engineApi, try_muted_autoplay) {
    var conf = freedomplayer.extend({
      startFragPrefetch: api.preload, // nice, loads m3u8 and one TS
      recoverMediaError: true
    }, player.conf.hlsjs, video.hlsjs);
    if (player.engine.hls) player.engine.hls.destroy();
    var hls = player.engine.hls = new Hls(conf);

    // Ensure HLS.js is destroyed when engine is unloaded (playlist switch or splash)
    if (!engineApi._hlsUnloadWrapped) {
      var originalUnload = engineApi.unload;
