...
flowplayer(function(api, root) {
  ...
  bean.on(document, "keydown.fp", function(e) {
    ...
    if (!metaKeyPressed && el.ready) {
      ...
      switch (key) {
        ...
        var current_subtitles = $(focusedRoot).find('.fp-dropdown li.active[data-subtitle-index]').data('subtitle-index');
  ...
  api.bind('ready', function(e,api,video) {
    if( video.subtitles && video.subtitles.length > 0 ) {
      var help = $(root).find('.fp-help').html();
      help += '<div class="fp-help-section fp-help-subtitles"><p><em>c</em>cycle through subtitles</p></div>';
      $(root).find('.fp-help').html(help);
    } else {
      $(root).find('.fp-help-subtitles').remove();
    }
  });
