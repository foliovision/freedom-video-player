...
freedomplayer(function(api, root) {
  ...
  bean.on(document, "keydown.fp", function(e) {
    ...
    // Allow tab key to skip to some other element
    if (key == 9) {
      return false;
    }
  ...
  // hover
  bean.on(root, "mouseenter mouseleave focus", function(e) {
    focused = !api.disabled && ( e.type == 'mouseover' || e.type == 'focus' ) ? api : 0;
