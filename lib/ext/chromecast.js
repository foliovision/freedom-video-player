...
freedomplayer(function(api, root) {
  ...
  // Only load for the first player instance
    ...
    cast.framework.CastContext.getInstance().addEventListener(
      ...
      function(event) {
        ...
        if ( event.castState !== cast.framework.CastState.NO_DEVICES_AVAILABLE ) {
          ...
          // Tell all the player instances about Chromecast being there
          var e = new CustomEvent("fv_player_chromecast_loaded");
  ...
  api.createChromecastButton = function() {
    ...
    var fs = common.find('.fp-fullscreen', root)[0];
  ...
  function destroy() {
    if ( remote_controller ) {
      remote_controller.removeEventListener( cast.framework.RemotePlayerEventType.ANY_CHANGE, onChromecastEvent );
    }
  ...
  document.addEventListener( 'fv_player_chromecast_loaded', function() {
    is_available = true;
  ...
  // When a video is loaded in another player instance, we need to stop the current one
  // TODO: It would be nicer if it would show the full player interface and not just play button
  document.addEventListener( 'fv_player_chromecast_video_loaded', function( e ) {
  ...
  api.bind('ready', function() {
    ...
    if( get_media() ) {
      ...
      if ( session ) {
        console.log( 'Chromecast resume session' );
        ...
        api.one( 'progress', function() {
          ...
          // seems like HLS.js with encrypted video needs a little time before api.loading = false
          setTimeout( function() {
            api.pause();
          });
  ...
  function onMediaDiscovered() {
    // Tell other player instances you are casting now
    var event = new CustomEvent( 'fv_player_chromecast_video_loaded', { detail: root.getAttribute('data-freedomplayer-instance-id') } );
    document.dispatchEvent(event);

    remote_player = new cast.framework.RemotePlayer();
    remote_controller = new cast.framework.RemotePlayerController( remote_player );

    remote_controller.addEventListener( cast.framework.RemotePlayerEventType.ANY_CHANGE, onChromecastEvent );
  }
