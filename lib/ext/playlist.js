...
freedomplayer(function(player, root) {
   ...
   player.play_next = function( video_index ) {
      if ( video_index > player.conf.playlist.length ) {
         video_index = player.conf.playlist.length - 1;
      }

      play_next_index = video_index;
   };
   ...
   player.have_visible_playlist = function() {
      var is_playlist = player.conf.playlist && player.conf.playlist.length > 0;

      if ( is_playlist ) {
         var count = 0;
         for ( var i in player.conf.playlist ) {
            if ( typeof player.conf.playlist[i].click == "undefined" ) {
               count++;
            }
         }
   ...
   bean.on(root, player.touch_events(), '.fp-next', player.next);
   bean.on(root, player.touch_events(), '.fp-prev', player.prev);
   ...
   player.off("finish.pl").on("finish.pl", function(e, player) {
      ...
      // Auto advance is disabled and no next clip is set
      if (!advance && play_next_index === -1 ) return;
      ...
      // Next clip is set by play_next()
      if ( play_next_index > -1 ) {
         next = play_next_index;
      }      
   ...
   player.on( 'ready', function( e, api, video ) {
      // Reset play_next index for the same video to not play again
      if ( play_next_index === api.video.index ) {
         play_next_index = -1;
      }
   })
