...
export function format(sec, remaining) {
...
function selectSVG(root) {
    ...
    if (isEdgy && isOutlined)
        return { play: PLAY_SHARP_OUTLINE, pause: PAUSE_SHARP_OUTLINE };
    if (isEdgy)
        return { play: PLAY_SHARP_FILL, pause: PAUSE_SHARP_FILL };
    if (isOutlined)
        return { play: PLAY_ROUNDED_OUTLINE, pause: PAUSE_ROUNDED_OUTLINE };
...
freedomplayer(function (api, root) {
    ...
    api.touch_events = function () {
        ...
        let el = root;
        for (let i = 0; i < 4; i++) {
            if (el && common.hasClass(el, 'mobx-item')) {
                touchEventsCache += ' touchend';
                break;
            }
    ...
    let hovertimer;
    let fullscreenExitHintTimer = false;
    ...
    function delayedCalls() {
        ...
        ui.style.display = '';
        if (placeholderVideo)
            placeholderVideo.style.display = '';
        if (conf.ratio)
            setRatio(conf.ratio);
        const bc = common.css(root, 'background-color');
        const hasBg = common.css(root, 'background-image') !== 'none' || (bc && bc !== 'rgba(0, 0, 0, 0)' && bc !== 'transparent');
        if (typeof conf.splash === 'string')
            common.css(root, 'background-image', "url('" + conf.splash + "')");
        if (!hasBg && api.forcedSplash)
    ...
    let speedAnimationTimers = [];
    function setRatio(val, adaptive) {
                        ...
                        if (!fullscreenExitHintTimer) {
                            fullscreenExitHintTimer = setTimeout(() => {
                                ...
                                setTimeout(() => {
                                    ...
                                    clearInterval(fullscreenExitHintTimer);
                                }, 2000);
                                clearInterval(fullscreenExitHintTimer);
    ...
    api.on('ready', (_ev, _api, video) => {
        ...
        if (conf.adaptiveRatio && video.height && video.width && !isNaN(video.height / video.width))
            setRatio(video.height / video.width, true);
        common.toggleClass(root, 'is-long', duration >= 3600);
        volumeApi.slide(api.volumeLevel);
        common.find('.fp-title', ui).forEach(common.removeNode);
        if (api.video.title) {
            common.prepend(ui, common.createElement('div', { className: 'fp-message fp-title' }, api.video.title));
        }
        common.toggleClass(root, 'has-title', !!api.video.title);
        common.html([durationEl, remaining], api.live ? 'Live' : format(duration));
    }).on('unload', () => {
        if (!origRatio && !conf.splash)
            common.css(ratio, 'paddingTop', '');
        if (api.ready)
            timelineApi.slide(0);
        common.addClass(play, 'fp-visible');
    }).on('buffer', (_ev, _api, buffered) => {
        const video = api.video;
        const max = (video.buffer || 0) / api.get_video_duration();
        if (!video.seekable && support.seekable)
            timelineApi.max(api.conf.live ? Infinity : max);
        if (video.seekable && support.seekable && timelineApi.getMax())
            timelineApi.max(1);
        ...
        buffered.forEach((b, i) => {
            ...
            if (b.end > dur)
                b.end = dur;
            if (api.rtl) {
                common.css(buffer, { right: (100 * b.start / dur) + '%', width: (100 * (b.end - b.start) / dur) + '%' });
            }
            else {
                common.css(buffer, { left: (100 * b.start / dur) + '%', width: (100 * (b.end - b.start) / dur) + '%' });
            }
            common.prepend(timeline, buffer);
        });
    }).on('speed', (_e, _api, val) => {
        if (api.video.time) {
        ...
        timelineApi.max(1);
    }).on('progress seek', (_e, _api, time) => {
        const duration = api.get_video_duration();
        const offset = api.video.seekOffset || 0;
        if (typeof time === 'undefined')
            time = api.video.time || 0;
    ...
    }).on('mute', (_e, _api, flag) => {
        common.toggleClass(root, 'is-muted', flag);
    }).on('error', (_e, _api, error) => {
        ...
        if (error) {
            ...
            if ((error.message || '').match(/DECODER_ERROR_NOT_SUPPORTED/))
                code = 3;
            const message = error.custom_message || (api.engine?.engineName || 'html5') + ': ' + conf.errors[code || 0];
            const dismiss = api.message(message);
            common.removeClass(root, 'is-mouseover');
            api.one('load progress', (e) => {
                if (e.type === 'progress' && !api.error)
                    dismiss();
            });
        }
    }).one('resume ready', () => {
        const videoTag = common.find('video.fp-engine', root)[0];
        if (!videoTag)
            return;
    ...
    // Interaction events
    domOn(root, 'mouseenter mouseleave', (e) => {
        if (noToggle || noControls)
            return;
    ...
    domOn(root, touchEvents, (e) => {
        if (api.disabled)
            return;
        const target = e.target;
        if (common.hasParent(target, '.fp-play,.fp-pause') ||
    ...
    domOn(root, 'mousemove touchmove', (e) => {
        const target = e.target?.closest('.fp-timeline');
        if (!target)
            return;
        const ev = e;
        let x = 'pageX' in ev ? ev.pageX : 0;
        if (!x && 'touches' in ev && ev.touches.length)
        ...
        if (api.fv_timeline_chapters_data?.[index]) {
            api.fv_timeline_chapters_data[index].forEach((chapter) => {
        ...
        if (left < 0)
            left = 0;
        if (left > common.width(timeline) - (timelineTooltip.offsetWidth || 0))
            left = false;
        if (left !== false)
            common.css(timelineTooltip, { left: left + 'px', right: 'auto' });
        else
            common.css(timelineTooltip, { left: 'auto', right: '0px' });
    });
    domOn(root, 'contextmenu', (ev) => {
        const menu = common.find('.fp-context-menu', root)[0];
        if (!menu)
            return;
        ev.preventDefault();
        const me = ev;
        api.showMenu(menu, { left: me.clientX - window.scrollX, top: me.clientY - window.scrollY });
        domOn(root, touchEvents, (ev2) => {
            if (ev2.target?.closest('.fp-context-menu'))
                ev2.stopPropagation();
        });
    });
    if (conf.poster)
        common.css(root, 'background-image', 'url(' + conf.poster + ')');
    if (!conf.splash) {
        if (!conf.poster)
            conf.poster = true;
        const initPoster = () => {
            ...
            common.addClass(play, 'fp-visible');
            api.poster = true;
            api.on('resume.poster progress.poster beforeseek.poster', (ev) => {
        ...
        api.on('ready', (_ev, _api, video) => {
            if (video.index || video.autoplay)
                return;
            initPoster();
        });
    }
    domOn(root, touchEvents, (ev) => {
        const target = ev.target?.closest('.fp-toggle, .fp-play, .fp-playbtn');
        if (!target || api.disabled)
            return;
        if (api.playing)
            api.manual_pause = true;
        else if (api.paused)
    ...
    domOn(root, touchEvents, (ev) => {
        ...
        if (target?.closest('.fp-fullscreen'))
            api.fullscreen?.();
        if (target?.closest('.fp-unload'))
            api.unload();
    });
    domOn(timeline, 'slide', (_val) => {
        const val = typeof _val === 'number' ? _val : (_val?.detail?.args?.[0] ?? 0);
        const duration = api.get_video_duration();
        const seekToSeconds = duration ? api.get_video_start() + val * duration : 0;
        api.seeking = true;
        api.manual_seeking = true;
        if (api.custom_seek)
            api.custom_seek(seekToSeconds);
        else
            api.seek(seekToSeconds);
    });
    domOn(volumeSlider, 'slide', (_val) => {
        const val = typeof _val === 'number' ? _val : (_val?.detail?.args?.[0] ?? 0);
        api.volume(val);
    });
    domOn(root, touchEvents, (ev) => {
        if (ev.target?.closest('.fp-duration,.fp-remaining')) {
            if (api.dvr) {
                api.seekTo(10);
                return;
            }
