...
freedomplayer(function (api, root) {
    if (!api.conf.airplay && !api.conf.skin_preview)
        return;
    api.on('ready', () => {
        const el = common.find('video.fp-engine', root)[0];
        if (!el)
            return;
        el.setAttribute('x-webkit-airplay', 'allow');
        if (!window.WebKitPlaybackTargetAvailabilityEvent)
            return;
        el.addEventListener('webkitplaybacktargetavailabilitychanged', (ev) => {
            if (ev.availability !== 'available' && !freedomplayer.airplay_was_available)
                return;
        ...
        api.one('progress', () => {
            const airplay = common.find('.fp-airplay', root)[0];
            if (!airplay)
                return;
    ...
    domOn(root, api.touch_events(), (ev) => {
        const target = ev.target?.closest('.fp-airplay');
        if (!target)
            return;
        ev.preventDefault();
    ...
    api.createAirplayButton = function () {
    ...
    if (api.conf.skin_preview && api.conf.airplay)
        api.createAirplayButton();
});
