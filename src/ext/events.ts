/**
 * Freedom Player event system
 *
 * Replaces bean library with native CustomEvent and a lightweight
 * namespace-based event manager. Supports:
 * - on(type, handler) / off(type) / one(type, handler)
 * - trigger(type, args)
 * - Namespaced events: "event.namespace"
 * - bind/unbind aliases
 */
...
export default function setupEvents(api: PlayerApi): void {
  const handlers: Map<string, HandlerEntry[]> = new Map();

  function parseType(raw: string): { event: string; ns: string } {
    const idx = raw.indexOf('.');
    if (idx === -1) return { event: raw, ns: '' };
    return { event: raw.substring(0, idx), ns: raw.substring(idx + 1) };
  }

  api.on = function (type: string, handler: (...args: unknown[]) => void) {
  ...
  api.one = function (type: string, handler: (...args: unknown[]) => void) {
    type.split(/\s+/).forEach(t => {
      const { event, ns } = parseType(t);
      if (!handlers.has(event)) handlers.set(event, []);
  ...
  api.trigger = function (type: string, args?: unknown[], _returnEvent?: boolean) {
...
/**
 * Lightweight DOM event helpers replacing bean for DOM element event binding.
 * Supports namespace-based off() for easy cleanup.
 */
interface DomHandlerEntry {
  ns: string;
  event: string;
  fn: EventListener;
...
const domStore = new WeakMap<EventTarget, DomHandlerEntry[]>();

function getStore(el: EventTarget): DomHandlerEntry[] {
  if (!domStore.has(el)) domStore.set(el, []);
  return domStore.get(el)!;
}

export function domOn(el: EventTarget, typeStr: string, handler: EventListener): void {
...
export function domOne(el: EventTarget, typeStr: string, handler: EventListener): void {
  typeStr.split(/\s+/).forEach(t => {
    const idx = t.indexOf('.');
    const event = idx === -1 ? t : t.substring(0, idx);
    const ns = idx === -1 ? '' : t.substring(idx + 1);
...
export function domOff(el: EventTarget, typeStr?: string): void {
  const store = getStore(el);
  if (!typeStr) {
...
export function domFire(el: EventTarget, event: string, args?: unknown[]): void {
 ...
 */
export function domDelegate(el: EventTarget, typeStr: string, selector: string, handler: (e: Event, matchedEl: Element) => void): void {
  domOn(el, typeStr, (e: Event) => {
    const target = (e.target as Element).closest(selector);
    if (target && (el as Element).contains?.(target)) {
      handler(e, target);
    }
  });
}
