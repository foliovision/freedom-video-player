  ...
  function try_muted_autoplay() {
    ...
    player.one('volume', function() {
      ...
      player.one('pause', function() {
        ...
        if (
          player.ready && ! player.playing && player.paused &&
          ( pausedTime - playbackStartTime <= 1000 )
        ) {

          // Mute the video and resume playback
          player.mute(true, true);
          api.play();
        }
      });
